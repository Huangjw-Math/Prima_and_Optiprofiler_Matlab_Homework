...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{Testing Report of the Prima and Optiprofiler Packages}
...
\begin{abstract}
    This is the testing report of the Prima and Optiprofiler packages.\\
    Firstly, We try to use Matlab Online(in Mathworks website, offering Linux environment) to test the two packages and complete the tasks, saving a great amount of time installing both the packages themselves and the prerequisites.\\
    However, due to the absence of parallel computing environment(discussed in details below), we are unable to complete the second task online, forcing us to turn back to local environment.
\end{abstract}



\section{The Prima Package}

\subsection{Installation}
    Download the Prima package from \url{https://github.com/libprima/prima}.
    Prerequisites: Matlab Mex
    run \('setup'\) in the \('prima'\) folder to install the package.
    Then run \('options.debug=true; options.single=true; options.quadruple=true; setup(options); testprima
    '\) to test the package.

\subsection{Testing}
    Run \('testprima_ex'\) in the \('prima/matlab/tests'\) folder to test the package.

\textbf{Issues:}
\begin{enumerate}
    \item The 'testprima_ex' will run the 'setup' function, which has already been done.
\end{enumerate}

\subsection{The Rosenbrock Function}
    Use prima to solve the following optimization problem in Matlab. One can refer to 'help prima' and the example in 'prima/matlab/examples'.
\textbf{Problem:}
    Object function: the Rosenbrock function in \(\mathbb{R}^{n}\).
    Initial point: \((-1, -1, \cdots, -1) \in \mathbb{R}^{n} \).
    Testing in different constraints:
\begin{enumerate}
    \item unconstrained
    \item Boundary constrained: \(x \leq 0\).
    \item linear constrained: \(\sum x \leq 1\), \(x \geq 0\).
    \item nonlinear constrained: \(\sum x^{2} \leq 1\), \(x \geq 0\).
\end{enumerate}

    \textbf{Answer From the Example:}
    Run ```testprima_ex```, we get the result in \(Example_Output.txt\).
    \tetxbf{Testing Output:}
    Run the code in \(Prima_Testing.m\), we get the result in \(Testing_Output.txt\).  

    Note that we have obtained different results of the same constraint, indicating that the choice of the initial point matters when the global minimum doesn't exist. If we replace the initial point with \(0; 0; 0\) in the testing code, we will obtain the same result as in the example code.

\section{The Optiprofiler Package}

\subsection{Installation}
    Use git to clone the repository. More specifically, run 'git clone https://github.com/optiprofiler/optiprofiler.git'.The repository is clone into 'optiprofiler'.
    Then run 'cd optiprofiler', followed by 'ls' to check if the copying process is successful.
    I forget where exactly the path in my local computer is, so I run 'pwd' to find the answer.
    Open the file you have cloned the repository to.
    Run 'setup' in the 'optiprofiler' folder to install the package.

\subsection{Testing}
    Run 'example 1; example 2; example 3; example 4;' in the 'optiprofiler/matlab/examples' folder to test the package.

\subsection{Testint Prima}
Complete to following two Matlab Testing tasks. One can refer to 'help benchmark' and the examples in \url{https: //www.optprof.com/latest/user/usage.html}.

\(\mathbb{Test 1:}\)
Solver 1: Prima, with precision 'double'.
Solver 2: Prima, with precision 'single'.
Testing Problem Type(ptype): 'ubln'(testing unconstrained, bound constrained, linear constrained, and nonlinear constrained problems altogether).
Testing Problem Dimension(mindim, maxdim): 2~20.
Testing Problem Feature(feature_name): 'plain' and 'noisy'(testing seperately).

\(\mathbb{Test 2:}\)
Solver 1: Prima, with precision 'double'.
Solver 2: Prima, with precision 'quadruple'.
Testing Problem Type(ptype): 'ubln'(testing unconstrained, bound constrained, linear constrained, and nonlinear constrained problems altogether).
Testing Problem Dimension(mindim, maxdim): 2~20.
Testing Problem Feature(feature_name): 'noisy'(testing only noisy problems).
